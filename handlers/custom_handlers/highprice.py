...
@bot.message_handler(state=HighPriceStates.ask_if_photos)
    ...
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:  # Контекстный манагер
            if 0 <= int(message.text) <= 4:
                ...
                return
            bot.set_state(message.from_user.id, HighPriceStates.make_request, message.chat.id)
...
@bot.message_handler(state=HighPriceStates.make_request)
def a_request(message: Message) -> None:
    if message.text == 'да':
        ...
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:  # Контекстный манагер
            # --- логирование
            log_string = f'{str(datetime.datetime.now())} /highprice\nГород: {data["city"].title()}\nНайденные отели:'
        ...
        for elem in get_from_txt.get_results(file_path)['data']['body']['searchResults']['results']:
            ...
            try:
                if 'found in' in request_anser:
                    ...
                    bot.send_message(message.from_user.id, f'Похоже, фотографий для {elem["name"]} не нашлось.\n'
                                                           f'{request_anser}')  # debug
                    photo_path = None
            except Exception:
                print(f'Фотографий для {elem["name"]} не нашлось.\n{request_anser}')
                bot.send_message(message.from_user.id, f'Фотографий для {elem["name"]} не нашлось.\n{request_anser}')
                photo_path = None
            if photo_path:
